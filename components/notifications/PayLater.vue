    ...
    <v-card-text class="pb-0">
      ...
      <div class="d-flex flex-sm-row flex-column align-center gray px-6 py-3 justify-space-between">
        ...
        <ul class="schema-list blue--text text-uppercase text-subtitle-2">
          <li v-for="item in payLaterData.listSchema" :key="'A' + item.id">{{ $t(item.name) }}</li>
        </ul>
      </div>
      <p class="mb-3 mt-4 text-subtitle-2">{{ $t(payLaterData.totalOrderSumTitle) }}</p>
      <ul class="pl-0 text-subtitle-2">
        <li class="pb-3" v-for="item in payLaterData.listOrderSum" :key="'B' + item.id" v-html="$t(item.name)"></li>
      </ul>
      <p class="mb-0 pt-2 text-subtitle-2" v-html="$t(payLaterData.payMethodSelected)"></p>
    </v-card-text>
    <v-card-actions>
      <v-spacer></v-spacer>
  ...
  props: {
    payLaterData: {
      ...
      default: () => ({
        ...
        listOrderSum: [
          { id: 1, name: 'NOTIFICATION_PAY_LATER_TOTAL_ORDER_SUM_LIST_TEXT_1' },
          { id: 2, name: 'NOTIFICATION_PAY_LATER_TOTAL_ORDER_SUM_LIST_TEXT_2' },
          { id: 3, name: 'NOTIFICATION_PAY_LATER_TOTAL_ORDER_SUM_LIST_TEXT_3' },
          { id: 4, name: 'NOTIFICATION_PAY_LATER_TOTAL_ORDER_SUM_LIST_TEXT_4' },
          { id: 5, name: 'NOTIFICATION_PAY_LATER_TOTAL_ORDER_SUM_LIST_TEXT_5' },
        ],
        payMethodSelected: 'NOTIFICATION_PAY_LATER_EASY_PAY_METHOD_SELECTED',
